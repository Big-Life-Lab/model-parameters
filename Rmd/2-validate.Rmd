...
| date::[date_format] | Use when the variable is a string that contains a date. The format of the date should be specified using [R notation](https://www.r-bloggers.com/2013/08/date-formats-in-r/) |
...
| age      | type   | number |
| sex      | type   | string |
| date_of_birth | type | date::[%d-%m-%Y] |

In the above validation CSV table,

* The age variable is a `number` type
* The sex variable is a `string` type
* The date_of_birth variable is a `date` type and is formatted as **%d-%m-%Y**, for example, **13-04-1989**.
